(function () {
  ...
  function applyPageContent(content) {
    ...
    if (pageContent.cards) document.querySelectorAll('.svc-card, .res-card, .card-grid .card').forEach((card, index) => { const item = pageContent.cards[index]; if (!item) return; const heading = card.querySelector('.svc-card__label, .res-card__title, .card__title'); const paragraph = card.querySelector('.svc-card__desc, .res-card__desc, .card__body-text'); const image = card.querySelector('img'); if (heading && item.heading) heading.textContent = item.heading; if (paragraph && item.paragraph) paragraph.textContent = item.paragraph; if (image && item.imageAlt) image.alt = item.imageAlt; });
    if (pageContent.heroSlides && pageContent.heroSlides[0] && document.querySelector('.page-hero__title')) { const item = pageContent.heroSlides[0]; const heading = document.querySelector('.page-hero__title'); const paragraph = document.querySelector('.page-hero__sub'); if (heading && item.heading) heading.textContent = item.heading; if (paragraph && item.paragraph) paragraph.textContent = item.paragraph; }
    { const heroCopy = document.querySelector('[class$="-hero__copy"], .sol-hero-landing'); if (pageContent.heroSlides && pageContent.heroSlides[0] && heroCopy) { const item = pageContent.heroSlides[0]; const heading = heroCopy.querySelector('h1'); const paragraph = heroCopy.querySelector('p'); const button = document.querySelector('[class$="-hero__btn"]'); if (heading && item.heading) heading.textContent = item.heading; if (paragraph && item.paragraph) paragraph.textContent = item.paragraph; if (button) { if (item.buttonText) button.textContent = item.buttonText; if (item.buttonUrl) button.href = item.buttonUrl; } } }
    if (pageContent.introduction) { const heading = setHeadingTag(document.querySelector('.main-services__title, .ab-intro__title, .svl-overview__title'), pageContent.introduction.headingTag); const paragraph = document.querySelector('.main-services__intro, .ab-intro p, .svl-overview__lead p, .svl-overview__lead'); if (heading && pageContent.introduction.heading) heading.textContent = pageContent.introduction.heading; if (paragraph && pageContent.introduction.paragraph) paragraph.textContent = pageContent.introduction.paragraph; }
    ...
    if (pageContent.heroSlides && pageContent.heroSlides[0] && document.querySelector('.csd-title')) { const item = pageContent.heroSlides[0]; const heading = document.querySelector('.csd-title'); const paragraph = document.querySelector('.csd-intro'); if (heading && item.heading) heading.textContent = item.heading; if (paragraph && item.paragraph) paragraph.textContent = item.paragraph; }
    if (pageContent.heroSlides && pageContent.heroSlides[0] && document.querySelector('.cs-hero-title')) { const item = pageContent.heroSlides[0]; const heading = document.querySelector('.cs-hero-title'); const paragraph = document.querySelector('.cs-hero-sub'); const button = document.querySelector('.cs-hero-cta'); if (heading && item.heading) heading.textContent = item.heading; if (paragraph && item.paragraph) paragraph.textContent = item.paragraph; if (button) { if (item.buttonText) button.textContent = item.buttonText; if (item.buttonUrl) button.href = item.buttonUrl; } }
    ...
    if (pageContent.contactCta) { const heading = setHeadingTag(document.querySelector('.nd-contact__title, .ctf-title, .industry-cta__title, .cs-center-panel p:first-of-type'), pageContent.contactCta.headingTag); const paragraph = document.querySelector('.nd-contact__intro, .ctf-copy, .svc-cta-band__text, .industry-cta__text, [class$="-cta-band__card"] p, .cs-center-panel p:nth-of-type(2)'); const button = document.querySelector('.svc-cta-band__btn, .industry-cta__card a, [class$="-cta-band__card"] a, .nd-contact__cta, .cs-center-panel a'); if (heading && pageContent.contactCta.heading) heading.textContent = pageContent.contactCta.heading; if (paragraph && pageContent.contactCta.paragraph) paragraph.textContent = pageContent.contactCta.paragraph; if (button) { if (pageContent.contactCta.buttonText) button.textContent = pageContent.contactCta.buttonText; if (pageContent.contactCta.buttonUrl) button.href = pageContent.contactCta.buttonUrl; } }
