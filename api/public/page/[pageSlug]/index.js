...
export default async function handler(request, response) {
  ...
  try {
    ...
    if (!page?.published_revision_id) return json(request, response, { page: null }, 200, { 'Cache-Control': 'public, s-maxage=5, stale-while-revalidate=30' });
    ...
    return json(request, response, { page: revision || null }, 200, { 'Cache-Control': 'public, s-maxage=5, stale-while-revalidate=30' });
  } catch (error) {
