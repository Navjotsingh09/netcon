(async function () {
  ...
  // Reusable H2/H3/H4 select for any singleton/repeater-item field, so every real section heading (not just FAQ) is tag-editable.
  function headingTagField(labelText, name, index, value, fieldName = 'headingTag', defaultTag = 'h2') {
    const current = value || defaultTag;
    return `<label>${escapeHtml(labelText)}<select data-repeater="${name}" data-index="${index}" data-field="${fieldName}"><option value="h2" ${current === 'h2' ? 'selected' : ''}>H2</option><option value="h3" ${current === 'h3' ? 'selected' : ''}>H3</option><option value="h4" ${current === 'h4' ? 'selected' : ''}>H4</option></select></label>`;
  }
  ...
  function setPageFormContent(content = {}) {
    ...
    elements.pageForm.elements.canonicalUrl.value = content['seo.canonicalUrl'] || '';
    elements.pageForm.elements.internalLinks.value = pageContent.internalLinks || '';
    elements.pageForm.elements.redirects.value = pageContent.redirects || '';
  ...
  function renderRepeaters(content = {}) {
    ...
    const acctIntro = pageContent.acctIntro || { eyebrow: '', heading: '', paragraph1: '', paragraph2: '', paragraph3: '', buttonText: '', buttonUrl: '' };
    const acctCards = pageContent.acctCards || [{ heading: '', paragraph: '', imageAlt: '' }, { heading: '', paragraph: '', imageAlt: '' }];
    const ravSection = pageContent.ravSection || { heading1: '', paragraph1: '', heading2: '', paragraph2: '', imageAlt: '' };
    const installationProcessLead = pageContent.installationProcessLead || { heading: '' };
    const installationProcessIntro = pageContent.installationProcessIntro || { heading: '', paragraph: '', imageAlt: '' };
    const installationProcess = pageContent.installationProcess || [{ title: '', description: '', imageUrl: '' }];
    const capabilities = pageContent.capabilities || [{ heading: '', paragraph: '', imageAlt: '' }, { heading: '', paragraph: '', imageAlt: '' }, { heading: '', paragraph: '', imageAlt: '' }];
    const storyCards = pageContent.storyCards || [{ heading: '', paragraph: '', imageAlt: '' }, { heading: '', paragraph: '', imageAlt: '' }];
    const proSlides = pageContent.proSlides || [{ title: '', bodyOne: '', bodyTwo: '', imageAlt: '' }, { title: '', bodyOne: '', bodyTwo: '', imageAlt: '' }, { title: '', bodyOne: '', bodyTwo: '', imageAlt: '' }];
    const contactReach = pageContent.contactReach || { heading: '', paragraph: '' };
    const contactCards = pageContent.contactCards || Array.from({ length: 6 }, () => ({ category: '', title: '', description: '' }));
    ...
    document.getElementById('why-choose-us-repeater').innerHTML = `<article class="cms-repeater-item"><label>Heading<input data-repeater="whyChooseUs" data-index="0" data-field="heading" value="${escapeHtml(whyChooseUs.heading || '')}" maxlength="180"></label>${headingTagField('Heading tag', 'whyChooseUs', 0, whyChooseUs.headingTag)}<label>Paragraph 1<textarea data-repeater="whyChooseUs" data-index="0" data-field="paragraph1" rows="3" maxlength="600">${escapeHtml(whyChooseUs.paragraph1 || '')}</textarea></label><label>Paragraph 2<textarea data-repeater="whyChooseUs" data-index="0" data-field="paragraph2" rows="3" maxlength="600">${escapeHtml(whyChooseUs.paragraph2 || '')}</textarea></label><label>Image URL<input data-repeater="whyChooseUs" data-index="0" data-field="imageUrl" value="${escapeHtml(whyChooseUs.imageUrl || '')}" maxlength="500"></label><label>Image alt text<input data-repeater="whyChooseUs" data-index="0" data-field="imageAlt" value="${escapeHtml(whyChooseUs.imageAlt || '')}" maxlength="250"></label></article>`;
    document.getElementById('why-choose-us-list-repeater').innerHTML = `<article class="cms-repeater-item"><label>Heading<input data-repeater="whyChooseUsList" data-index="0" data-field="heading" value="${escapeHtml(whyChooseUsList.heading || '')}" maxlength="180"></label>${headingTagField('Heading tag', 'whyChooseUsList', 0, whyChooseUsList.headingTag)}<label>Bullets (one per line)<textarea data-repeater="whyChooseUsList" data-index="0" data-field="itemsText" rows="8" maxlength="1200">${escapeHtml(whyChooseUsList.itemsText || '')}</textarea></label></article>`;
    document.getElementById('common-issues-repeater').innerHTML = `<article class="cms-repeater-item"><label>Heading<input data-repeater="commonIssues" data-index="0" data-field="heading" value="${escapeHtml(commonIssues.heading || '')}" maxlength="220"></label>${headingTagField('Heading tag', 'commonIssues', 0, commonIssues.headingTag)}<label>Intro<textarea data-repeater="commonIssues" data-index="0" data-field="paragraph" rows="3" maxlength="600">${escapeHtml(commonIssues.paragraph || '')}</textarea></label><label>Tile labels (one per line)<textarea data-repeater="commonIssues" data-index="0" data-field="itemsText" rows="10" maxlength="1200">${escapeHtml(commonIssues.itemsText || '')}</textarea></label></article>`;
    document.getElementById('accountable-repeater').innerHTML = `<article class="cms-repeater-item"><label>Eyebrow label<input data-repeater="acctIntro" data-index="0" data-field="eyebrow" value="${escapeHtml(acctIntro.eyebrow || '')}" maxlength="80"></label><label>Heading<input data-repeater="acctIntro" data-index="0" data-field="heading" value="${escapeHtml(acctIntro.heading || '')}" maxlength="180"></label>${headingTagField('Heading tag', 'acctIntro', 0, acctIntro.headingTag)}<label>Intro paragraph 1<textarea data-repeater="acctIntro" data-index="0" data-field="paragraph1" rows="3" maxlength="500">${escapeHtml(acctIntro.paragraph1 || '')}</textarea></label><label>Intro paragraph 2<textarea data-repeater="acctIntro" data-index="0" data-field="paragraph2" rows="3" maxlength="500">${escapeHtml(acctIntro.paragraph2 || '')}</textarea></label><label>Intro paragraph 3<textarea data-repeater="acctIntro" data-index="0" data-field="paragraph3" rows="3" maxlength="500">${escapeHtml(acctIntro.paragraph3 || '')}</textarea></label><label>Button text<input data-repeater="acctIntro" data-index="0" data-field="buttonText" value="${escapeHtml(acctIntro.buttonText || '')}" maxlength="100"></label><label>Button URL<input data-repeater="acctIntro" data-index="0" data-field="buttonUrl" value="${escapeHtml(acctIntro.buttonUrl || '')}" maxlength="300"></label></article>` + acctCards.map((item, index) => `<article class="cms-repeater-item"><div class="cms-repeater-item__head"><strong>Card ${index + 1}</strong></div><label>Heading<input data-repeater="acctCards" data-index="${index}" data-field="heading" value="${escapeHtml(item.heading || '')}" maxlength="180"></label><label>Paragraph<textarea data-repeater="acctCards" data-index="${index}" data-field="paragraph" rows="4" maxlength="700">${escapeHtml(item.paragraph || '')}</textarea></label><label>Image alt text<input data-repeater="acctCards" data-index="${index}" data-field="imageAlt" value="${escapeHtml(item.imageAlt || '')}" maxlength="250"></label></article>`).join('');
    document.getElementById('remote-access-value-repeater').innerHTML = `<article class="cms-repeater-item"><label>Heading 1<input data-repeater="ravSection" data-index="0" data-field="heading1" value="${escapeHtml(ravSection.heading1 || '')}" maxlength="180"></label>${headingTagField('Heading 1 tag', 'ravSection', 0, ravSection.heading1Tag, 'heading1Tag', 'h3')}<label>Paragraphs under heading 1 (leave a blank line between paragraphs)<textarea data-repeater="ravSection" data-index="0" data-field="paragraph1" rows="4" maxlength="800">${escapeHtml(ravSection.paragraph1 || '')}</textarea></label><label>Heading 2<input data-repeater="ravSection" data-index="0" data-field="heading2" value="${escapeHtml(ravSection.heading2 || '')}" maxlength="180"></label>${headingTagField('Heading 2 tag', 'ravSection', 0, ravSection.heading2Tag, 'heading2Tag', 'h3')}<label>Paragraphs under heading 2 (leave a blank line between paragraphs)<textarea data-repeater="ravSection" data-index="0" data-field="paragraph2" rows="6" maxlength="1200">${escapeHtml(ravSection.paragraph2 || '')}</textarea></label><label>Image alt text<input data-repeater="ravSection" data-index="0" data-field="imageAlt" value="${escapeHtml(ravSection.imageAlt || '')}" maxlength="250"></label></article>`;
    document.getElementById('installation-process-repeater').innerHTML = `<article class="cms-repeater-item"><label>Lead heading (above the process tabs)<input data-repeater="installationProcessLead" data-index="0" data-field="heading" value="${escapeHtml(installationProcessLead.heading || '')}" maxlength="220"></label>${headingTagField('Heading tag', 'installationProcessLead', 0, installationProcessLead.headingTag)}<label>Section heading<input data-repeater="installationProcessIntro" data-index="0" data-field="heading" value="${escapeHtml(installationProcessIntro.heading || '')}" maxlength="180"></label>${headingTagField('Heading tag', 'installationProcessIntro', 0, installationProcessIntro.headingTag)}<label>Section intro<textarea data-repeater="installationProcessIntro" data-index="0" data-field="paragraph" rows="3" maxlength="600">${escapeHtml(installationProcessIntro.paragraph || '')}</textarea></label><label>Shared image alt text<input data-repeater="installationProcessIntro" data-index="0" data-field="imageAlt" value="${escapeHtml(installationProcessIntro.imageAlt || '')}" maxlength="250"></label></article>` + installationProcess.map((item, index) => `<article class="cms-repeater-item"><div class="cms-repeater-item__head"><strong>Tab ${index + 1}</strong></div><label>Title<input data-repeater="installationProcess" data-index="${index}" data-field="title" value="${escapeHtml(item.title || '')}" maxlength="120"></label><label>Description<textarea data-repeater="installationProcess" data-index="${index}" data-field="description" rows="3" maxlength="500">${escapeHtml(item.description || '')}</textarea></label><label>Image URL<input data-repeater="installationProcess" data-index="${index}" data-field="imageUrl" value="${escapeHtml(item.imageUrl || '')}" maxlength="500"></label><label>Image alt text (defaults to title if left blank)<input data-repeater="installationProcess" data-index="${index}" data-field="imageAlt" value="${escapeHtml(item.imageAlt || '')}" maxlength="250"></label></article>`).join('');
    document.getElementById('capabilities-repeater').innerHTML = capabilities.map((item, index) => `<article class="cms-repeater-item"><div class="cms-repeater-item__head"><strong>Card ${index + 1}</strong></div><label>Heading<input data-repeater="capabilities" data-index="${index}" data-field="heading" value="${escapeHtml(item.heading || '')}" maxlength="180"></label><label>Paragraph<textarea data-repeater="capabilities" data-index="${index}" data-field="paragraph" rows="3" maxlength="600">${escapeHtml(item.paragraph || '')}</textarea></label><label>Image alt text<input data-repeater="capabilities" data-index="${index}" data-field="imageAlt" value="${escapeHtml(item.imageAlt || '')}" maxlength="250"></label></article>`).join('');
    document.getElementById('story-cards-repeater').innerHTML = storyCards.map((item, index) => `<article class="cms-repeater-item"><div class="cms-repeater-item__head"><strong>Story card ${index + 1}</strong></div><label>Heading<input data-repeater="storyCards" data-index="${index}" data-field="heading" value="${escapeHtml(item.heading || '')}" maxlength="180"></label><label>Paragraph(s) (leave a blank line between paragraphs)<textarea data-repeater="storyCards" data-index="${index}" data-field="paragraph" rows="5" maxlength="1200">${escapeHtml(item.paragraph || '')}</textarea></label><label>Image alt text<input data-repeater="storyCards" data-index="${index}" data-field="imageAlt" value="${escapeHtml(item.imageAlt || '')}" maxlength="250"></label></article>`).join('');
    document.getElementById('pro-slides-repeater').innerHTML = proSlides.map((item, index) => `<article class="cms-repeater-item"><div class="cms-repeater-item__head"><strong>Slide ${index + 1}</strong></div><label>Title<input data-repeater="proSlides" data-index="${index}" data-field="title" value="${escapeHtml(item.title || '')}" maxlength="180"></label><label>Body paragraph 1<textarea data-repeater="proSlides" data-index="${index}" data-field="bodyOne" rows="3" maxlength="600">${escapeHtml(item.bodyOne || '')}</textarea></label><label>Body paragraph 2<textarea data-repeater="proSlides" data-index="${index}" data-field="bodyTwo" rows="3" maxlength="600">${escapeHtml(item.bodyTwo || '')}</textarea></label><label>Image alt text<input data-repeater="proSlides" data-index="${index}" data-field="imageAlt" value="${escapeHtml(item.imageAlt || '')}" maxlength="250"></label></article>`).join('');
    document.getElementById('contact-reach-repeater').innerHTML = `<article class="cms-repeater-item"><label>Heading<input data-repeater="contactReach" data-index="0" data-field="heading" value="${escapeHtml(contactReach.heading || '')}" maxlength="180"></label><label>Paragraph<textarea data-repeater="contactReach" data-index="0" data-field="paragraph" rows="3" maxlength="600">${escapeHtml(contactReach.paragraph || '')}</textarea></label></article>`;
    document.getElementById('contact-cards-repeater').innerHTML = contactCards.map((item, index) => `<article class="cms-repeater-item"><div class="cms-repeater-item__head"><strong>Tile ${index + 1}</strong></div><label>Category label<input data-repeater="contactCards" data-index="${index}" data-field="category" value="${escapeHtml(item.category || '')}" maxlength="60"></label><label>Title<input data-repeater="contactCards" data-index="${index}" data-field="title" value="${escapeHtml(item.title || '')}" maxlength="120"></label><label>Description<textarea data-repeater="contactCards" data-index="${index}" data-field="description" rows="3" maxlength="400">${escapeHtml(item.description || '')}</textarea></label></article>`).join('');
  ...
  function updateSectionLegends(pageContent, heroSlides) {
    ...
    setLegend('accountable-repeater', pageContent.acctIntro?.heading);
    setLegend('remote-access-value-repeater', pageContent.ravSection?.heading1);
    setLegend('installation-process-repeater', pageContent.installationProcessIntro?.heading);
    setLegend('testimonial-repeater', pageContent.testimonialsHeading);
  }
  const SINGLETON_REPEATERS = new Set(['introduction', 'contactCta', 'highlight', 'keySolutionAreasIntro', 'whoWeServe', 'businessImpactIntro', 'businessImpactLead', 'workProcessIntro', 'impactStat', 'whyChooseUs', 'whyChooseUsList', 'commonIssues', 'acctIntro', 'ravSection', 'installationProcessLead', 'installationProcessIntro', 'contactReach']);
  ...
  async function loadStaticPageContent(slug) {
    ...
    // Read the page's current live SEO tags so the editor opens pre-filled with what's actually published, instead of blank fields that look like nothing was ever set.
    const seoDefaults = {
      ...
      'seo.ogTitle': doc.querySelector('meta[property="og:title"]')?.getAttribute('content') || '',
      'seo.ogDescription': doc.querySelector('meta[property="og:description"]')?.getAttribute('content') || '',
      'seo.canonicalUrl': doc.querySelector('link[rel="canonical"]')?.getAttribute('href') || '',
      'seo.schemaMarkup': doc.querySelector('script[type="application/ld+json"]')?.textContent.trim() || ''
    };
    // Hero background is a <video poster>, not an <img> -- read the poster frame, not the video's own .mp4 source.
    const slides = [...doc.querySelectorAll('.hero__slide')].map((slide) => ({ heading: text('.hero__title', slide), paragraph: text('.hero__sub', slide), buttonText: text('.hero__btn', slide), buttonUrl: slide.querySelector('.hero__btn')?.getAttribute('href') || '', imageUrl: slide.querySelector('video')?.getAttribute('poster') || slide.querySelector('img')?.getAttribute('src') || '', imageAlt: slide.querySelector('img')?.alt || '' }));
    ...
    if (slug === 'home') {
      ...
      content.cards = [...doc.querySelectorAll('.svc-card')].map((card) => ({ heading: text('.svc-card__label', card), paragraph: text('.svc-card__desc', card), imageAlt: card.querySelector('img')?.alt || '' }));
      ...
      content.contactCta = { heading: text('.nd-contact__title'), paragraph: text('.nd-contact__intro'), buttonText: text('.nd-contact__cta'), buttonUrl: doc.querySelector('.nd-contact__cta')?.getAttribute('href') || '' };
      content.capabilities = [...doc.querySelectorAll('.ab-cap-card')].map((card) => ({ heading: text('.ab-cap-card__title', card), paragraph: text('.ab-cap-card__body', card), imageAlt: card.querySelector('img')?.alt || '' }));
      content.storyCards = [...doc.querySelectorAll('.ab-story-card')].map((card) => ({ heading: text('.ab-story-card__title', card), paragraph: [...card.querySelectorAll('.ab-story-card__body')].map((node) => node.textContent.replace(/\s+/g, ' ').trim()).join('\n\n'), imageAlt: card.querySelector('img')?.alt || '' }));
      // proSlides is a runtime-only JS array (drives the rotating "Our Engineers' Proficiency" slider), not present in the static DOM.
      const proSlidesData = extractArrayLiteral(doc, 'var proSlides', /var proSlides\s*=\s*(\[[\s\S]*?\]);/);
      if (proSlidesData) content.proSlides = proSlidesData.map((item) => ({ title: item.title || '', bodyOne: item.bodyOne || '', bodyTwo: item.bodyTwo || '', imageAlt: item.imageAlt || '' }));
    } else if (slug === 'contact') {
      content.contactReach = { heading: text('.ctu-title'), paragraph: text('.ctu-body p') };
      content.contactCards = [...doc.querySelectorAll('.ctu-card')].map((card) => ({ category: text('.ctu-cat', card), title: text('.ctu-card-title', card), description: text('.ctu-desc', card) }));
      ...
      const ksaData = extractArrayLiteral(doc, "getElementById('ksa')", /var data\s*=\s*(\[[\s\S]*?\]);/)
        ...
        || [];
      content.keySolutionAreasIntro = { heading: text('#ksa-h'), paragraph: text('#ksa-h + p'), imageAlt: doc.getElementById('ksa-img')?.getAttribute('alt') || '' };
      content.keySolutionAreas = [...doc.querySelectorAll('#ksa .ksa__tab')].map((tab, index) => ({ title: tab.textContent.replace(/\s+/g, ' ').trim(), description: ksaData[index]?.desc || '', imageUrl: ksaData[index]?.img || '', imageAlt: ksaData[index]?.alt || '' }));
      ...
      content.businessImpactLead = { heading: bizLeadHeadingEl ? bizLeadHeadingEl.textContent.replace(/\s+/g, ' ').trim() : '', paragraph: text('#biz-lead-h ~ p') };
      ...
      // Managed Network Support only: "A Managed Service That is Truly Accountable" section, not covered by any shared repeater above.
      if (doc.querySelector('#acct-h')) {
        const acctParagraphs = [...doc.querySelectorAll('.acct__intro > p')];
        const acctCta = doc.querySelector('.acct__cta');
        content.acctIntro = {
          eyebrow: text('.acct__eyebrow'),
          heading: text('#acct-h'),
          paragraph1: acctParagraphs[0]?.textContent.replace(/\s+/g, ' ').trim() || '',
          paragraph2: acctParagraphs[1]?.textContent.replace(/\s+/g, ' ').trim() || '',
          paragraph3: acctParagraphs[2]?.textContent.replace(/\s+/g, ' ').trim() || '',
          buttonText: acctCta?.textContent.replace(/\s+/g, ' ').trim() || '',
          buttonUrl: acctCta?.getAttribute('href') || ''
        };
        content.acctCards = [...doc.querySelectorAll('.acct__card')].map((card) => ({
          heading: card.querySelector('h3')?.textContent.replace(/\s+/g, ' ').trim() || '',
          paragraph: card.querySelector('p')?.textContent.replace(/\s+/g, ' ').trim() || '',
          imageAlt: card.querySelector('img')?.getAttribute('alt') || ''
        }));
      }

      // Remote Access & VPN only: the two-heading "Keep Your Business Running..." / "Secure Your Company Data..." value section.
      const ravHeadings = [...doc.querySelectorAll('.rav__text h3')];
      if (ravHeadings.length) {
        const ravImage = doc.querySelector('.rav__media img');
        const paragraphsBetween = (startHeading, endHeading) => {
          const paragraphs = [];
          let node = startHeading?.nextElementSibling;
          while (node && node !== endHeading) {
            if (node.tagName === 'P') paragraphs.push(node.textContent.replace(/\s+/g, ' ').trim());
            node = node.nextElementSibling;
          }
          return paragraphs.join('\n\n');
        };
        content.ravSection = {
          heading1: ravHeadings[0]?.textContent.replace(/\s+/g, ' ').trim() || '',
          paragraph1: paragraphsBetween(ravHeadings[0], ravHeadings[1]),
          heading2: ravHeadings[1]?.textContent.replace(/\s+/g, ' ').trim() || '',
          paragraph2: paragraphsBetween(ravHeadings[1], null),
          imageAlt: ravImage?.getAttribute('alt') || ''
        };
      }

      // Network Installations only: the second tabbed "Our Network Installation Process" widget (#ksa2), separate from the shared #ksa widget above.
      if (doc.querySelector('#ksa2')) {
        const ksa2Data = extractArrayLiteral(doc, "initKsa('ksa2',", /initKsa\('ksa2',\s*'ksa2',\s*(\[[\s\S]*?\])\);/) || [];
        content.installationProcessLead = { heading: text('.adv-lead h2') };
    ...
    const orderMarkers = [
      ...
      ['services-list-repeater', doc.querySelector('.svl-overview__list-item')],
      ...
      // The live markup uses either a bullet-list variant ("Our process includes:", .whyc__list) or a two-paragraph variant (.whyc__content p) under the same #whyc-h heading, never both — mark the section id that actually matches so the correct fieldset (not the other one) becomes visible in the admin.
      ['why-choose-us-list-repeater', doc.querySelector('.whyc__list')],
    ...
    if (slug === 'business-continuity-and-network-resilience') {
      ...
      content.seoDefaults = seoDefaults;
      return content;
    }
  ...
  async function openPage(slug) {
    ...
    try {
      ...
      setPageFormContent({ ...staticFallback.seoDefaults, content: staticFallback });
    ...
    // Same per-key backfill for the SEO fields: only fall back to the live page's real meta/canonical/schema when the draft/published revision has nothing saved for that field yet.
    Object.entries(staticFallback.seoDefaults || {}).forEach(([key, value]) => {
    ...
    // Enabled as soon as the page is open (not gated on an existing draft): generatePreviewLink() always saves the current form content first, so it works on a brand-new page too.
    if (elements.pagePreviewLink) elements.pagePreviewLink.disabled = false;
