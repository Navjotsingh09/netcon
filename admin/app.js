  ...
  async function loadStaticPageContent(slug) {
    ...
    if (slug === 'home') {
      ...
      if (doc.querySelector('#ksa2')) {
        ...
        content.installationProcess = [...doc.querySelectorAll('#ksa2 .ksa__tab')].map((tab, index) => ({ title: tab.textContent.replace(/\s+/g, ' ').trim(), description: ksa2Data[index]?.desc || '', imageUrl: ksa2Data[index]?.img || '' }));
      }
    } else {
